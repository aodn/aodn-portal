...
describe("Portal.visualise.animations.TemporalExtentParser", function() {
    ...
    it('expandExtendedDates Correct compressed Request', function() {

        var res = temporalExtentParser.expandExtendedDates([
    ...
    it('expandExtendedDates Correct Simple Request', function() {
        // uncompressed dates go through unaltered
        var res = temporalExtentParser.expandExtendedDates(['2001-01-10T22:36:00.000Z']);
        expect(res[0]).toBeSame('2001-01-10T22:36:00.000Z');
    });

    it('end date is included', function() {
        expect(temporalExtentParser._expandExtendedDate('2000-01-01T00:00:00.000Z/2000-01-01T01:00:00.000Z/PT30M').length).toBe(3);
    });

    it('expandExtendedDates Invalid Start date Request', function() {
        var res = temporalExtentParser.expandExtendedDates(['2001-59-10T22:36:00.000Z/2001-01-12T21:48:00.000Z/PT23H36M']);
        expect(res.length).toBe(0);
    });

    it('expandExtendedDates Invalid parameter Request', function() {
    ...
    describe('temporalExtent._getPeriod', function() {
        ...
        it('understands a mixed period of months hours minutes and seconds', function() {
            var res = temporalExtentParser._getPeriod('P5MT23H36M24S');
        ...
        it('understands a mixed period of months  minutes and seconds', function() {
            var res = temporalExtentParser._getPeriod('P5MT36M24S');
            var expectedResult = _initPeriod({ "seconds": 24, "minutes": 36, "months": 5 });
            expect(res).toEqual(expectedResult);
        });

        it('understands a full period', function() {
            var res = temporalExtentParser._getPeriod('P2Y5M1W2DT6H36M24S');
