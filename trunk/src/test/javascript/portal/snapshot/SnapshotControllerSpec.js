describe("Portal.snapshot.SnapshotController", function()
{
  ...
  var snapshotController = new Portal.snapshot.SnapshotController({map: mockMap});
  ...
  it("calls proxy save method with snapshot when createSnapshot is called", function() {
    ...
    snapshotController.createSnapshot('test snapshot save',null,null);
    
    expect(snapshotController.proxy.save).toHaveBeenCalled();
    
    var snapshot = snapshotController.proxy.save.mostRecentCall.args[0];
    
    expect(snapshot.name).toEqual('test snapshot save');
    expect(snapshot.owner).toEqual(Portal.app.config.currentUser.id);
