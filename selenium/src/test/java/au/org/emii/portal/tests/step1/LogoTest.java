...
public class LogoTest extends BaseTest {
    ...
    private void testStyle(String style) {
        ...
        for (String item: styleItems) {
            ...
            if (property.equals("max-height")) {
                Assert.assertTrue(
                        pair[1].equals(this.MAX_DIMENSION + "px"),
                        "max-height (style) expected to be at least " + this.MAX_DIMENSION + " px"
                );
            }
            else if (property.equals("max-width")) {
                Assert.assertTrue(
                        pair[1].equals(this.MAX_DIMENSION + "px"),
                        "max-width (style) expected to be at least " + this.MAX_DIMENSION + " px"
                );
            }
        }
    }
    ...
    private static List<Integer> generatePageList(int count, int min, int max) {
        Random random = new Random();
        List<Integer> ints = new ArrayList<Integer>();
        for (int i = 0; i < count; i++) {
            ints.add(Integer.valueOf(random.nextInt(max - min + 1) + min));
        }
